...
const MyRoutes = () => {
  ...
  const showRoutes = () =>
    data.user[shownRoutes]
      .map(route => <LargeRouteCard data={route} key={route._id} />)

  return (
    <>
      <header class={style.header}>
        <nav>
          <button onClick={() => setShownRoutes('owned_routes')}
            class={shownRoutes === 'owned_routes' ? style.selected : ''}
          >
            OWNED ROUTES
          </button>
          <button onClick={() => setShownRoutes('saved_routes')}
